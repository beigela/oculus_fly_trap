...
public class FlyPhysicsMore : MonoBehaviour {
	...
	private int posneg;
	private int prevPosneg;

	// Use this for initialization
	void Start () {
		spriteRenderer = renderer as SpriteRenderer;
		//origin = new Vector3(1, 1, 1);
		origin = new Vector3(UnityEngine.Random.Range (-0.5f, 1.0f),
		                     UnityEngine.Random.Range (0.3f, 1.3f),
		                     UnityEngine.Random.Range (-2.0f, 1.0f));
		revSpeed = 80;
	...
	void Update () {
		...
		if(updateCount % 60 == 0)
		{
			...
			rotPoint = transform.position - new Vector3(UnityEngine.Random.Range (-2.5f, 2.5f),
		...
		{
			if (prevPosneg != posneg)
			{
				Turn (rotPoint, 0.3f*posneg*Vector3.up);
			}
			else
			{
				Turn(rotPoint, posneg*Vector3.up);
			}
			Debug.Log ("in turn: " + rotPoint);
		}
		else
		{
			//transform.RotateAround(rotPoint, posneg* Vector3.up, revSpeed * Time.deltaTime);
			Turn(rotPoint, posneg*Vector3.up);
		}
	...
	private void Turn(Vector3 refVec, Vector3 axis)
	{
		transform.RotateAround(refVec, axis, revSpeed * Time.deltaTime);
	}
